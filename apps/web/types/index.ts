...
export const ExperienceSchema = z.object({
  ...
  end_date: z
    ...
    .regex(
      /^\d{4}-\d{2}$|^Atual$/,
      'End date must be in YYYY-MM format or "Atual"',
    ),
...
export const LanguageSchema = z.object({
  ...
  level: z.enum(["A2", "B1", "B2", "C1", "C2", "Nativo"], {
    errorMap: () => ({
      message: "Level must be one of: A2, B1, B2, C1, C2, Nativo",
    }),
  }),
});
...
export const ResumeResponseSchema = z.object({
  ...
  candidate_introduction: z
    .string()
    .min(1, "Candidate introduction is required"),
  experiences: z
